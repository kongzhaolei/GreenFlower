...
public class ByteArrayConveter {
	...
	public static int getInt(byte[] arr, int index,Boolean swap) {
		if(swap)
		return (0xff000000 & (arr[index + 0] << 24))
				| (0x00ff0000 & (arr[index + 1] << 16))
				| (0x0000ff00 & (arr[index + 2] << 8))
			...
			return (0xff000000 & (arr[index + 2] << 24))
	...
	public static float getFloat(byte[] arr, int index,Boolean swap) {
		return Float.intBitsToFloat(getInt(arr, index,swap));
	}
