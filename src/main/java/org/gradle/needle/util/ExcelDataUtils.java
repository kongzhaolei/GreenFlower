...
public class ExcelDataUtils {
	...
	public static void ParserAndCompare(String testid, String bsheet, String response, String csheet) {
		...
		try {
			...
			String expectstr = (String) ExcelDataUtils.getCellData(brownum, expectcol);
			...
			while (iter.hasNext()) {
				...
				if (avalue != null) {
					...
					if (avalue.equals(evalue)) {
						ExcelDataUtils.setCellData("passed", crownum, Resultcol);
						ExcelDataUtils.setBorder(crownum, Resultcol);
						ExcelDataUtils.setCellColor("GREEN", crownum, Resultcol);
					} else {
